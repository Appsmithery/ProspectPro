...
# Detect actual repo root dynamically
if [[ -z "${PROSPECTPRO_REPO_ROOT:-}" ]]; then
  if DETECTED_ROOT=$(git rev-parse --show-toplevel 2>/dev/null); then
    PROSPECTPRO_REPO_ROOT="$DETECTED_ROOT"
  else
    PROSPECTPRO_REPO_ROOT=/workspaces/ProspectPro
  fi
fi

DEFAULT_SUPABASE_PROJECT_REF=${SUPABASE_PROJECT_REF:-sriycekxdqnesdsgwiuc}
SUPABASE_DIR=${SUPABASE_DIR:-${PROSPECTPRO_REPO_ROOT}/app/backend}
